...
LOGOUTPUTFILE="$LOGDIRPATH"/"qualys_detections_changed.log"

# where fetch_new_detections_xml.sh stored the raw data from the Qualys API
QUALYSAPIOUTPUT="detections_changed.xml"

TMPDATAFILE="detections_changed.tmp"
...
then
  echo
  echo converting XML of changed detections to log lines
  echo
  xsltproc --nonet --novalid --output $TMPDATAFILE detections_changed_line_by_line.xslt $QUALYSAPIOUTPUT
fi
